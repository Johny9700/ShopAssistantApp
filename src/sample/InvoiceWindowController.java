...
public class InvoiceWindowController {
    ...
    void okButtonClicked(ActionEvent actionEvent) {
        ...
        try{
            if (!nip.isEmpty()) {
                //TODO odporność na błędy
                nipLong = Long.valueOf(nip);
                if (nipLong < 0 || nip.length() != 10) {
                    throw new NumberFormatException("bad number");
                }
            }
            String adres = addressTextField.getText();
            if (nip.isEmpty() && adres.isEmpty()) {
                GlobalData.insertNewSaleEvidenceP(receiptNumber, null, null, Double.parseDouble(adequatePrice), boughtProduct);
            } else if (nip.isEmpty() && !adres.isEmpty()) {
                GlobalData.insertNewSaleEvidenceP(receiptNumber, null, adres, Double.parseDouble(adequatePrice), boughtProduct);
            } else if (!nip.isEmpty() && adres.isEmpty()) {
//            System.out.println(nipLong);
                GlobalData.insertNewSaleEvidenceP(receiptNumber, nipLong, null, Double.parseDouble(adequatePrice), boughtProduct);
            } else {
//            System.out.println(nipLong);
                GlobalData.insertNewSaleEvidenceP(receiptNumber, nipLong, adres, Double.parseDouble(adequatePrice), boughtProduct);
            }
            ...
            quitAlert.setTitle("Wystawiono fakturę");
            quitAlert.setHeaderText("Wystawiono fakturę o numerze: " + receiptNumber);
            ...
            wrongTelephoneNumberFormat.setTitle("Niepoprawny nip");
            wrongTelephoneNumberFormat.setHeaderText(null);
            wrongTelephoneNumberFormat.setContentText("Niepoprawny nip.\nProszę wprowadzić poprawny numer nip.");
